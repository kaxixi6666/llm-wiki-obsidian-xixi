...
- **边界**：
  - 原始资料存放于 `raw/`，**只读不改**
  - 对于论文类资料，默认优先从 `raw/research_papers/` 读取 PDF
...
### 原始资料层
- `raw/`：原始资料根目录，**只读**
- `raw/research_papers/`：论文 PDF 的统一存放目录，默认从这里读取论文
- `raw/web_clips/`：网页 Markdown、网页摘录、保存的网页文本等（可选）
- `raw/images/`：图片、截图、图表等原始资料（可选）
- `raw/other/`：暂时无法归类的原始资料（可选）
